...
describe('computeDefaultViewport', () => {
  ...
  it('pads around peak range', () => {
    const pred: NmrPrediction = {
      ...
      solvent: 'CDCl3',
      peaks: [
        { atom_index: 0, atom_indices: [0], parent_indices: [0], shift_ppm: 2.0, integral: 1, multiplicity: 's', coupling_hz: [], environment: 'methyl', confidence: 3, method: 'additive' },
        { atom_index: 1, atom_indices: [1], parent_indices: [1], shift_ppm: 7.0, integral: 1, multiplicity: 's', coupling_hz: [], environment: 'aromatic', confidence: 3, method: 'additive' },
      ],
      metadata: { engine_version: '0.2.0', data_version: null, method: 'additive' },
    };
    const vp = computeDefaultViewport(pred);
