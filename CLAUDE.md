...
4. `cd backend && uv run ruff check .` (zero errors)
5. `cd backend && uv run ruff format --check .` (zero reformats)
6. `cd backend && uv run mypy kendraw_api/ kendraw_chem/ kendraw_settings/ kendraw_observability/` (zero errors)
7. `cd backend && uv run pytest -v` (zero failures)
