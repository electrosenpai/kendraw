...
async function main() {
  ...
  console.log('| SMILES (truncated)                       | Parse (ms) | Canon (ms) | InChI (ms) |');
  console.log('|------------------------------------------|------------|------------|------------|');
  for (const r of results) {
    console.log(`| ${r.smiles.padEnd(40)} | ${r.parseMs.toFixed(3).padStart(10)} | ${r.canonMs.toFixed(3).padStart(10)} | ${r.inchiMs.toFixed(3).padStart(10)} |`);
  }
  ...
  console.log(`Parse < 200 ms:      ${maxParse.toFixed(3)} ms → ${maxParse < 200 ? 'PASS' : 'FAIL'}`);
  console.log(`(First interactive < 2 s on 4G: requires browser test, estimated ${(loadTime + 500).toFixed(0)} ms on broadband)`);
}
